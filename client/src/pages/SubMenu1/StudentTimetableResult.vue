      ...
      <tbody>
        <tr v-for="(label, index) in periodLabels" :key="index">
          ...
          <td v-for="day in days" :key="day">
            <div v-for="item in getCell(day, index + 1)" :key="item.teacher_name + item.subject_name"
              class="cell-entry">
  ...
  methods: {
    async fetchSchedule() {
      try {
        ...
        const res = await axios.get(`http://localhost:3000/api/students/${studentId}/Schedule`, {
    ...
    getCell(day, periodIndex) {
      return this.schedule.filter(
        (item) => item.day_of_week === day && item.period_id === periodIndex
      );
    }
